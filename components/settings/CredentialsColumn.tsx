...
export function CredentialsColumn() {
  ...
  function publishStatus(next: SettingsView) {
    setSettingsStatus({ anthropic: next.anthropic.configured, gemini: next.gemini.configured });
  }
    ...
    <>
      <section className="settings-section">
        <div className="settings-section-head">
          ...
          <span className="status-pill ok">{demoDataset === 'logistics' ? 'logistics' : 'generic'}</span>
        </div>
        <label className="tweak-toggle" style={{ fontSize: 12.5, color: 'var(--ink-2)' }}>
          <input type="radio" name="demoDataset" checked={demoDataset === 'default'} onChange={() => setTweak('demoDataset', 'default')} />
          <span>Generic company <span style={{ color: 'var(--ink-4)', fontSize: 11.5 }}>· Acme Cloud</span></span>
        </label>
        <label className="tweak-toggle" style={{ fontSize: 12.5, color: 'var(--ink-2)' }}>
          <input type="radio" name="demoDataset" checked={demoDataset === 'logistics'} onChange={() => setTweak('demoDataset', 'logistics')} />
          <span>Crestview Freight <span style={{ color: 'var(--ink-4)', fontSize: 11.5 }}>· Supply chain</span></span>
        </label>
      </section>
